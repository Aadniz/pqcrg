...
from library.pqcrypto.pqcrypto.kem import saber
import socket

from .communication import Communication


class SaberKEMAESCBCEncryption(Communication):
    def __init__(self, transport_layer):
        super().__init__(transport_layer)
        self.pubkey, self.privkey = saber.generate_keypair()
    ...
    def decrypt(self, message: bytes, host: str = None) -> bytes:
        if host is None:
            raise Exception(f"SaberKEM-AES requires host to be known when decrypting!")
        if host not in self._connections:
            raise Exception(f"{host} does not have a public key set!")
        key = self._connections[host]["key"]
    ...
    def send_handshake(self, peer: tuple[str, int]):
        ...
        ciphertext = self._sock.recv(1088)
        #print(f"[CLIENT]: Got ciphertext {ciphertext}")
        plaintext_recovered = saber.decrypt(self.privkey, ciphertext)
