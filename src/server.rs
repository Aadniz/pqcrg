...
impl Server {
    ...
    fn listen_udp(self: &Arc<Self>) {
        ...
        loop {
                ...
                match crypter::decrypt(shared_secret.clone(), nonce, ciphertext) {
                    Ok(data) => match String::from_utf8(data) {
                        Ok(string) => {
                            ...
                            if messages_to_receive.contains(&string.as_str()) {
                                received_messages.insert(string);
                            }
                        }
            ...
            if received_messages.len() == messages_to_receive.len() {
                println!(
                    "Done: {}",
                    SystemTime::now()
