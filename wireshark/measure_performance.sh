...
protocol="$(python -c "import sys; sys.path.append('..'); import settings; print(f'{settings.TRANSPORT_LAYER}')")"
...
do
  ...
  port=$(python -c "import random; print(random.randint(7522, 8522))")
  ...
  if [ "$protocol" = "tcpp" ]; then
    ...
    echo "Running $protocol test $i into file $file"
    if python ../main.py "$port" > "$file"; then
      i=$((i+1))
    else
      echo "main.py failed, deleting $file"
      rm "$file"
